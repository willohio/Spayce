...
- (void)commonInit {
    ...
    self.font = [UIFont spc_boldSystemFontOfSize:13];
    ...
    self.selectionIndicatorBoxOpacity = 1.0f;
    ...
    if (self.type == HMSegmentedControlTypeText) {
        [self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
            ...
            if (self.segmentWidthStyle == HMSegmentedControlSegmentWidthStyleFixed) {
                ...
                // New look - Spayce Offset
                if (1 < [self.sectionTitles count]) {
                    if (0 == idx) {
                        // rect x offset equals segmentwidth minus 23pt padding minus stringWidth
                        rect = CGRectMake(self.segmentWidth - ([[self class] firstLastSegmentOffsetPercent] * self.segmentWidth) - stringWidth, y, stringWidth, stringHeight);
                    } else if ([self.sectionTitles count] - 1 == idx) {
                        // rect x offset equals segmentwidth*idx plus 23pt padding minus stringWidth/2
                        rect = CGRectMake((self.segmentWidth * idx) + ([[self class] firstLastSegmentOffsetPercent] * self.segmentWidth), y, stringWidth, stringHeight);
                    }
                }
            ...
            titleLayer.frame = CGRectIntegral(rect);
...
- (CGRect)frameForSelectionIndicator {
    ...
    if (self.selectionStyle == HMSegmentedControlSelectionStyleArrow) {
        ...
        if (self.selectionStyle == HMSegmentedControlSelectionStyleTextWidthStripe &&
            ...
            if (1 < [self.sectionTitles count]) { // New look - Spayce offset
                if (0 == self.selectedSegmentIndex) {
                    CGFloat x = self.segmentWidth - ([[self class] firstLastSegmentOffsetPercent] * self.segmentWidth) - [[self.sectionTitles objectAtIndex:0] sizeWithAttributes:@{NSFontAttributeName:self.font}].width;
                ...
                } else if ([self.sectionTitles count] - 1 == self.selectedSegmentIndex) {
                    CGFloat x = (([self.sectionTitles count] - 1) * self.segmentWidth) + ([[self class] firstLastSegmentOffsetPercent] * self.segmentWidth);
                    return CGRectMake(x + self.selectionIndicatorEdgeInsets.left, indicatorYOffset, sectionWidth - self.selectionIndicatorEdgeInsets.right, self.selectionIndicatorHeight);
                } else {
                    CGFloat widthToEndOfSelectedSegment = (self.segmentWidth * self.selectedSegmentIndex) + self.segmentWidth;
                    CGFloat widthToStartOfSelectedIndex = (self.segmentWidth * self.selectedSegmentIndex);
...
#pragma mark - New look - Spayce offset

+ (CGFloat)firstLastSegmentOffsetPercent {
    return 65.0f/270.0f; // Offset this far in % from the nearest segment edge - #pxPadding/#pxSegment
}
