...
- (id)init
{
    ...
    if (self) {
        ...
        _font = [UIFont spc_regularSystemFontOfSize:14];
        _height = 56.0;
        _selectionIndicatorHeight = 2.0;
        _selectionIndicatorOffset = 0.0;
        ...
        [_containerView addSubview:_selectionIndicator];
        
        _hairline = [UIView new];
        _hairline.backgroundColor = [UIColor clearColor];
...
- (void)layoutSubviews
{
    ...
    _containerView.frame = CGRectMake(
                                      CGRectGetMinX(self.bounds) + self.contentInsets.left,
    ...
    for (int i = 0; i < [self buttons].count; i++) {
        ...
        [button setFrame:CGRectMake(roundf(self.containerView.bounds.size.width/self.numberOfSegments)*i, 0, roundf(self.containerView.bounds.size.width/self.numberOfSegments), self.containerView.bounds.size.height)];
...
- (UIColor *)titleColorForState:(UIControlState)state
{
    ...
    if (!color) {
        switch (state) {
            case UIControlStateNormal:              return [UIColor colorWithWhite:105.0f/255.0f alpha:1.0f];
            case UIControlStateHighlighted:         return [UIColor colorWithWhite:105.0f/255.0f alpha:1.0f];
            case UIControlStateDisabled:            return [UIColor colorWithWhite:105.0f/255.0f alpha:1.0f];
            case UIControlStateSelected:            return [UIColor colorWithWhite:105.0f/255.0f alpha:1.0f];
            default:                                return [UIColor colorWithWhite:105.0f/255.0f alpha:1.0f];
        }
    }
...
- (CGRect)selectionIndicatorRect
{
    ...
    frame.origin.y = (_barPosition > UIBarPositionBottom) ? 0.0 : (button.frame.size.height-_selectionIndicatorHeight-_selectionIndicatorOffset);
...
- (void)setTitleColor:(UIColor *)color forState:(UIControlState)state
{
    ...
    for (UIButton *button in [self buttons]) {
        ...
        if (_showsCount) {
            ...
            UIColor *hackColor = color;
            if (state == UIControlStateSelected) {
                hackColor = [self titleColorForState:UIControlStateNormal];  //used because we don't want title colors to match bar color
            }
            [attributedString addAttribute:NSForegroundColorAttributeName value:hackColor range:NSMakeRange(0, attributedString.string.length)];
        }
...
{
    //NSLog(@"DZN setSelected for segment  %i",(int)segment);
    ...
    //NSLog(@"DZN animate to selection at index: %i",(int)_selectedSegmentIndex);
