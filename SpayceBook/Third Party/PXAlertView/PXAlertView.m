...
- (id)initAlertWithView:(UIView *)contentView completion:(void(^) (BOOL cancelled))completion
{
    self = [super init];
    if (self) {
        ...
        if (!_alertWindow) {
            ...
            [_alertWindow setAutoresizesSubviews:YES];
        }
        ...
        _alertView.autoresizingMask = UIViewAutoresizingFlexibleHeight | UIViewAutoresizingFlexibleWidth;
...
- (id)initAlertWithView:(UIView *)contentView cancelTitle:(NSString *)cancelTitle completion:(void(^) (BOOL cancelled))completion
{
    self = [super init];
    if (self) {
        [self setAutoresizesSubviews:YES];
        ...
        lineLayer.frame = CGRectMake(0, _contentView.frame.size.height-AlertViewButtonHeight-.5, AlertViewWidth, 0.5);
        ...
        _cancelButton.backgroundColor = [UIColor colorWithRed:94/255.0 green:196/255.0 blue:221/255.0 alpha:1.0];
        ...
        _cancelButton.frame = CGRectMake(0, _contentView.frame.size.height-AlertViewButtonHeight, AlertViewWidth, AlertViewButtonHeight);
        ...
        //fix LandscapeRight upside down problem
...
- (id)initAlertWithView:(UIView *)contentView
...
- (id)initBottomAlertWithView:(UIView *)contentView
                  cancelTitle:(NSString *)cancelTitle
                cancelBgColor:(UIColor *)cancelBgColor
              cancelTextColor:(UIColor *)cancelTextColor
                  cancelFrame:(CGRect)frame
                   completion:(void(^) (BOOL cancelled))completion
{
    ...
    if (self) {
        ...
        self.frame = _alertWindow.bounds;
        _alertView.bounds = CGRectMake(0, 0, self.bounds.size.width, contentView.frame.size.height);
        ...
        _alertView.frame= CGRectMake((self.bounds.size.width-_contentView.frame.size.width)/2, self.bounds.size.height-_alertView.bounds.size.height, _contentView.frame.size.width, _contentView.frame.size.height);
        _contentView.center = CGPointMake(_alertView.bounds.size.width/2, _alertView.bounds.size.height-(_contentView.bounds.size.height/2));
             ...
             completion:(void(^) (BOOL cancelled))completion
{
    ...
    if (self) {
        ...
        _otherButton.backgroundColor = otherBgColor;
        _otherButton.titleLabel.font = [UIFont boldSystemFontOfSize:17];
        [_otherButton setTitleColor:otherTextColor forState:UIControlStateNormal];
...
+ (PXAlertView *)showAlertWithView:(UIView *)view completion:(void(^) (BOOL cancelled))completion
{
    PXAlertView *alertView = [[PXAlertView alloc] initAlertWithView:view completion:completion];
    [alertView show];
    return alertView;
}
...
+ (PXAlertView *)showAlertWithView:(UIView *)view
                        ...
                        completion:(void(^) (BOOL cancelled))completion
{
    PXAlertView *alertView = [[PXAlertView alloc] initAlertWithView:view
                                                        cancelTitle:(NSString *)cancelTitle
                                                         completion:completion];
    [alertView show];
    return alertView;
...
+ (PXAlertView *)showBottomAlertWithView:(UIView *)view
                         ...
                         cancelTextColor:(UIColor *)cancelTextColor
                             cancelFrame:(CGRect)frame
                              completion:(void(^) (BOOL cancelled))completion
{
    PXAlertView *alertView = [[PXAlertView alloc] initBottomAlertWithView:view
                                                              cancelTitle:(NSString *)cancelTitle
                                                            cancelBgColor:(UIColor *)cancelBgColor
                                                          cancelTextColor:(UIColor *)cancelTextColor
                                                              cancelFrame:(CGRect)frame
                        ...
                        completion:(void (^)(BOOL))completion
{
    PXAlertView *alertView = [[PXAlertView alloc] initAlertWithView:view completion:completion];
    [alertView show];
    [alertView performSelector:@selector(dismiss:) withObject:alertView afterDelay:delay];
    return alertView;
}
