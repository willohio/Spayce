...
@synthesize scrollView, imageView, cropOverlayView, resizableCropArea, xOffset, yOffset, showCircleMask;
...
- (void)setCropSize:(CGSize)cropSize{
    ...
    if (self.cropOverlayView == nil){
        ...
        else if (self.showCircleMask && self.venueCrop) {
            float adjW=300;
            if ([[UIDevice currentDevice] userInterfaceIdiom] == UIUserInterfaceIdiomPad)  {
                adjW=600;
            }
            CGRect circleCropFrame = CGRectMake((self.bounds.size.width-adjW)/2, (self.bounds.size.height-adjW)/2, adjW, adjW);
            self.cropOverlayView = [[GKResizeableCropOverlayView alloc] initWithFrame:circleCropFrame andInitialContentSize:CGSizeMake(circleCropFrame.size.width, circleCropFrame.size.height)];
        }
        else
    ...
    if (self.showCircleMask) {
        self.cropOverlayView.hidden = YES;
    
        UIImageView *circleMaskImgView =  [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, self.bounds.size.width, self.bounds.size.width)];
        circleMaskImgView.center = CGPointMake(self.center.x,self.center.y);
        UIImage *circleImg = [UIImage imageNamed:@"camera-overlay"];
        circleMaskImgView.image=circleImg;
        circleMaskImgView.backgroundColor=[UIColor clearColor];
        circleMaskImgView.userInteractionEnabled=NO;
        [self addSubview:circleMaskImgView];

        UIView *topShade = [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.bounds.size.width, circleMaskImgView.frame.origin.y)];
        topShade.backgroundColor=[UIColor colorWithRed:0 green:0 blue:0 alpha:.7];
        topShade.userInteractionEnabled=NO;
        [self addSubview:topShade];
        
        UIView *botShade = [[UIView alloc] initWithFrame:CGRectMake(0, circleMaskImgView.frame.origin.y+circleMaskImgView.frame.size.height, self.bounds.size.width, self.bounds.size.height-(circleMaskImgView.frame.origin.y+circleMaskImgView.frame.size.height))];
        botShade.backgroundColor=[UIColor colorWithRed:0 green:0 blue:0 alpha:.7];
        botShade.userInteractionEnabled=NO;
        [self addSubview:botShade];
    }
}
...
- (UIImage *)croppedImage{
    ...
    if (self.showCircleMask && self.venueCrop) {
        visibleRect  = [self _calcVisibleRectForVenueCircleCropArea];
    }
...
- (CGRect)_calcVisibleRectForVenueCircleCropArea{
    GKResizeableCropOverlayView* resizeableView = (GKResizeableCropOverlayView*)self.cropOverlayView;
...
-(CGRect)_calcVisibleRectForCropArea{
    ...
    // Edits from https://github.com/gekitz/GKImagePicker/issues/29 and
    // https://github.com/gekitz/GKImagePicker/issues/23
...
- (void)layoutSubviews{
    ...
    // Edits from here: https://github.com/gekitz/GKImagePicker/issues/29
